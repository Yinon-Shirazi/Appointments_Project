...
<?php
	...
	$status = false;
	 if (strtotime("now") > strtotime($row[0]))  // checking if the start of the time appointment has passed.
		$status = true;
	else
		$status = false;
  ...
  ?>
	...
	<form action="timeslots.php" method="post">
		...
		<!-- Hiding button for past time slots -->
		<div <?php if($status == true) {?> class="submitHide" <?php } else {?> class="submitShow" <?php } ?>>  
