...
?>
    ...
    ?>
        <h1 style="margin:auto;"> <br>NEW USER INFORMATION</h1>
        </br>
        <form action="information.php" method="post">
            <div class="firstName">
                <label for="firstName"><span style="color:#20B2AA; padding-left:40px;"> First Name: </label>
                <input type="text" id="firstName" name="firstName" value= <?php echo $firstName ?> >
            </div>
            <br>
            <div class="lastName">
                <label for="lastName"  style="color:#20B2AA; padding-left:40px;">Last Name: </label>
                <input type="text" id="lastName" name="lastName" value= <?php echo $lastName ?>>
            </div>
            <br>
            <div class="info">
                <label for="email" style="color:#20B2AA; padding-left:84px;">Email: </label>
                <input type="text" id="email" name="email" value= <?php echo $email?>>
                <br><br>
                <label for="phone" style="color:#20B2AA; padding-left:0px;">Phone Number: </label>
				...
				$conn = mysqli_connect("localhost","root", "", "CSC350GroupCTerm");
					...
					else // writing a query into a variable.
						$sql = "select cast(APT as unsigned) as APT from userinfo order by APT;";
						$result = mysqli_query($conn, $sql);
						$taken = mysqli_fetch_array($result);
					//Stores any apartment number that is not taken into the rooms array
					for($i = 0; $i < 25; $i++)
					{
						if ($i+1 == $taken[0])
							$taken = mysqli_fetch_array($result);
					...
					//Generates a drop down option for every apartment number that is not taken
					for($j = 0; $rooms[$j]; $j++)
					{
            ...
            if ($Validated)
            {
                ...
                echo ' <br> <a style="color: white" href="login.php">Login To Authenticate User</a>';
            }
             ?>
        </form>
    </div>
