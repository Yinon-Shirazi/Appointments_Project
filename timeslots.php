...
<?php
	...
	//sets deleting to 1 if it receives the information to delete from appointment.php
	...
	if ($deleting)
	{
		$sql = "delete from schedule where sessionid = ".$todelete;
		$result = mysqli_query($conn, $sql);
	}
	//receives the two arrays dateData and boolDate from the session
    session_start();
...
<?php
	...
	//Stores dates from dateData in a month-day format into array dateDataMD
	...
	$endDateSTR = date_format($endDate,"YmdHis");
		...
		// writing a query into a variable.
		$sql = "select usedate from schedule where usedate > ".$startDateSTR." AND usedate < ".$endDateSTR." ORDER BY usedate;";
		$result = mysqli_query($conn, $sql);
...
<?php
//Creates rows and tables for each timeslot of each day of the week, placing Not Available in timeslots that has been passed or taken, and a radio button next to the timeslot for timeslots that are available
 echo'<tr>';
	$y = 0;
	for ($x = 0; $x < 7; $x++)
	{
...
?>
	...
	<input type="hidden" name="postdelete" value="
	<?php
	if ($deleting)
		echo $todelete;
	else
	...
	"/>
    <br>
    <input style="margin-left:44%;width:13.5%; height:5%;background:#1abc9c; font-size:1.5em;" type="submit" value="SELECT">
</form>
